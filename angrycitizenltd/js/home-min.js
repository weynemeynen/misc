...
/*!function(b) {
	b.fn.ellipsis = function(a) {
		var d = {row: 1, onlyFullWords: !1, "char": "...", callback: function() {
		}, position: "tail"};
		return a = b.extend(d, a), this.each(function() {
			var F = b(this), E = F.text(), D = E, C = D.length, B = F.height();
			F.text("a");
			var A = parseFloat(F.css("lineHeight"), 10), z = F.height(), y = A > z ? A - z : 0, x = y * (a.row - 1) + z * a.row;
			if (x >= B) {
				return F.text(E), a.callback.call(this), void 0
			}
			var w = 1, v = 0, u = E.length;
			if ("tail" === a.position) {
				for(; u > w;) {
				...
				if ("middle" === a.position) {
					...
					t = Math.max(C - w, 0);
					var s = D.slice(0, Math.floor((C - t) / 2)), r = D.slice(Math.floor((C + t) / 2), C);
...
(function(c) {
	...
	c.fn.cycle = function(d) {
		...
		return 0 !== this.length || c.isReady ? this.each(function() {
			var m, h, j, k, i = c(this), g = c.fn.cycle.log;
			if (!i.data("cycle.opts")) {
				...
				h = c.extend({}, c.fn.cycle.defaults, m, d || {}), h.timeoutId = 0, h.paused = h.paused || !1, h.container = i, h._maxZ = h.maxZ, h.API = c.extend({_container: i}, c.fn.cycle.API), h.API.log = g, h.API.trigger = function(n, l) {
		...
		}) : (e = {s: this.selector, c: this.context}, c.fn.cycle.log("requeuing slideshow (dom not ready)"), c(function() {
			c(e.s, e.c).cycle(d)
		}), this)
	}, c.fn.cycle.API = {opts: function() {
		return this._container.data("cycle.opts")
	}, addInitialSlides: function() {
		var e = this.opts(), d = e.slides;
		...
		var e, d = this.opts(), f = d.container;
		...
		var d = this.opts(), g = d.API.getSlideOpts(), f = d.hoverPaused || d.paused;
		...
		var f = this.opts(), d = !f.hoverPaused && !f.paused;
		...
		var j, f = this.opts(), g = f.slideCount, h = !1;
		...
		var j = m, d = c(h), g = c(f), k = function() {
			g.animate(j.animIn || {opacity: 1}, j.speed, j.easeIn || j.easing, e)
		};
		...
		var g = this.opts(), f = void 0 !== d ? d : e.timeout;
		...
		var d = this.opts();
		if (d._initialized) {
			var j = d.API.getSlideOpts(), g = d.slides[d.currSlide];
		...
		var h, j = f.maxZ - 2, g = f.slideCount;
		if (k) {
		...
		return "Cycle2: " + a
	}, c.fn.cycle.transitions = {custom: {}, none: {before: function(g, f, d, h) {
		g.API.stackSlides(d, f, h), g.cssBefore = {opacity: 1, display: "block"}
	}}, fade: {before: function(e, d, h, f) {
		var g = e.API.getSlideOpts(e.nextSlide).slideCss || {};
		e.API.stackSlides(d, h, f), e.cssBefore = c.extend(g, {opacity: 0, display: "block"}), e.animIn = {opacity: 1}, e.animOut = {opacity: 0}
	}}, fadeout: {before: function(e, d, h, f) {
		var g = e.API.getSlideOpts(e.nextSlide).slideCss || {};
		e.API.stackSlides(d, h, f), e.cssBefore = c.extend(g, {opacity: 1, display: "block"}), e.animOut = {opacity: 0}
	}}, scrollHorz: {before: function(h, f, d, j) {
		h.API.stackSlides(f, d, j);
		var g = h.container.css("overflow", "hidden").width();
		h.cssBefore = {left: j ? g : -g, top: 0, opacity: 1, display: "block"}, h.cssAfter = {zIndex: h._maxZ - 2, left: 0}, h.animIn = {left: 0}, h.animOut = {left: j ? -g : g}
	}}}, c.fn.cycle.defaults = {allowWrap: !0, autoSelector: ".cycle-slideshow[data-cycle-auto-init!=false]", delay: 0, easing: null, fx: "fade", hideNonActive: !0, loop: 0, manualFx: void 0, manualSpeed: void 0, manualTrump: !0, maxZ: 100, pauseOnHover: !1, reverse: !1, slideActiveClass: "cycle-slide-active", slideClass: "cycle-slide", slideCss: {position: "absolute", top: 0, left: 0}, slides: "> img", speed: 500, startingSlide: 0, sync: !0, timeout: 4000, updateView: -1}, c(document).ready(function() {
		...
		if ("container" == h) {
			...
			if (k._autoHeightRatio) {
				...
				if ("calc" === h || "number" == d.type(h) && h >= 0) {
					...
					k._sentinelIndex = j, k._sentinel && k._sentinel.remove(), g = d(k.slides[j].cloneNode(!0)), g.removeAttr("id name rel").find("[id],[name],[rel]").removeAttr("id name rel"), g.css({position: "static", visibility: "hidden", display: "block"}).prependTo(k.container).addClass("cycle-sentinel cycle-slide").removeClass("cycle-slide-active"), g.find("*").css("visibility", "hidden"), k._sentinel = g
				}
			}
		}
	}
	...
	function f(g, e, l, h) {
		var j = d(h).outerHeight(), k = e.sync ? e.speed / 2 : e.speed;
		e.container.animate({height: j}, k)
	}
	...
	d.extend(d.fn.cycle.defaults, {autoHeight: 0}), d(document).on("cycle-initialized", function(h, m) {
		function n() {
			b(h, m)
		}

		var j, g = m.autoHeight, e = d.type(g), k = null;
	...
	a.extend(a.fn.cycle.defaults, {caption: "> .cycle-caption", captionTemplate: "{{slideNum}} / {{slideCount}}", overlay: "> .cycle-overlay", overlayTemplate: "<div>{{title}}</div><div>{{desc}}</div>", captionModule: "caption"}), a(document).on("cycle-update-view", function(c, b, e, d) {
		"caption" === b.captionModule && a.each(["caption", "overlay"], function() {
			var g = this, f = e[g + "Template"], h = b.API.getComponent(g);
			h.length && f ? (h.html(b.API.tmpl(f, e, b, d)), h.show()) : h.hide()
		})
	}), a(document).on("cycle-destroyed", function(c, b) {
		var d;
		a.each(["caption", "overlay"], function() {
			var g = this, f = b[g + "Template"];
	...
	}, b.extend(b.fn.cycle, a), b.extend(a.API, {next: function() {
		var d = this.opts();
	...
	a.extend(a.fn.cycle.defaults, {loader: !1}), a(document).on("cycle-bootstrap", function(c, b) {
		function e(h, m) {
			function j(l) {
				var n;
			...
			f && (h.hide().appendTo("body").each(function(p) {
				...
				var n = 0, q = a(this), o = q.is("img") ? q : q.find("img");
	...
	function a(h, f) {
		var d = this.opts();
		if (!d.busy || d.manualTrump) {
			var k = h.children().index(f), g = k, j = g > d.currSlide;
			d.currSlide != g && (d.nextSlide = g, d.API.prepareTx(!0, j), d.API.trigger("cycle-pager-activated", [d, h, f]))
		}
	}

	c.extend(c.fn.cycle.defaults, {pager: "> .cycle-pager", pagerActiveClass: "cycle-pager-active", pagerEvent: "click.cycle", pagerTemplate: "<span>&bull;</span>"}), c(document).on("cycle-bootstrap", function(f, d, g) {
		g.buildPagerLink = b
	...
	a.extend(a.fn.cycle.defaults, {next: "> .cycle-next", nextEvent: "click.cycle", disabledClass: "disabled", prev: "> .cycle-prev", prevEvent: "click.cycle", swipe: !1}), a(document).on("cycle-initialized", function(d, c) {
		if (c.API.getComponent("next").on(c.nextEvent, function(g) {
			...
			var b = c.swipeVert ? "swipeUp.cycle" : "swipeLeft.cycle swipeleft.cycle", f = c.swipeVert ? "swipeDown.cycle" : "swipeRight.cycle swiperight.cycle";
		...
		if (!d.allowWrap) {
			var b = d.disabledClass, k = d.API.getComponent("next"), f = d.API.getComponent("prev"), h = d._prevBoundry || 0, j = void 0 !== d._nextBoundry ? d._nextBoundry : d.slideCount - 1;
	...
	a.extend(a.fn.cycle.defaults, {progressive: !1}), a(document).on("cycle-pre-initialize", function(p, h) {
		if (h.progressive) {
			var f, q, e = h.API, k = e.next, b = e.prev, g = e.prepareTx, m = a.type(h.progressive);
			if ("array" == m) {
				f = h.progressive
			} else {
	...
	a.extend(a.fn.cycle.defaults, {tmplRegex: "{{((.)?.*?)}}"}), a.extend(a.fn.cycle.API, {tmpl: function(c, b) {
		var e = RegExp(b.tmplRegex || a.fn.cycle.defaults.tmplRegex, "g"), d = a.makeArray(arguments);
...
/*(function(a) {
	a(document).on("cycle-bootstrap", function(d, c, b) {
		"carousel" === c.fx && (b.getSlideIndex = function(h) {
			var g = this.opts()._carouselWrap.children(), f = g.index(h);
			return f % g.length
		}, b.next = function() {
			var f = c.reverse ? -1 : 1;
	...
	}), a.fn.cycle.transitions.carousel = {preInit: function(c) {
		...
		var g = e.carouselVisible || e.slides.length, j = {display: b ? "block" : "inline-block", position: "static"};
		if (e.container.css({position: "relative", overflow: "hidden"}), e.slides.css(j), e._currSlide = e.currSlide, h = a('<div class="cycle-carousel-wrap"></div>').prependTo(e.container).css({margin: 0, padding: 0, top: 0, left: 0, position: "absolute"}).append(e.slides), e._carouselWrap = h, b || h.css("white-space", "nowrap"), e.allowWrap !== !1) {
			for(k = 0; (void 0 === e.carouselVisible ? 2 : 1) > k; k++) {
		...
		var c, h, e, g = d.carouselVertical, b = d.carouselVisible || d.slides.length;
		...
		var e, g = d.slides.eq(0), b = g.outerWidth() - g.width(), f = this.prepareDimensions;
		a(window).on("resize", c), d._carouselResizeThrottle = c, h()
	}, transition: function(j, w, g, x, f) {
		var h, b = {}, m = j.nextSlide - j.currSlide, q = j.carouselVertical, k = j.speed;
		if (j.allowWrap === !1) {
			x = m > 0;
			var v = j._currSlide, e = j.slideCount - j.carouselVisible;
		...
		return function() {
			var f = a(c.slides[c.nextSlide]).position(), g = 0 - f[e ? "top" : "left"] + (c.carouselOffset || 0);
...
(function(f) {
	...
	function k() {
		...
		f(".extra-news a").each(function() {
			...
			f(this).text(l).ellipsis({row: 3})
		})
	}

	var g = {expandHorz: function(l) {
		f(".panel-title").stop(true, true).fadeOut(200);
		l.stop().removeClass("compressed").addClass("expanded").animate({width: "64%"}, 700);
		f(".panel-content", l).stop(true, true).delay(400).fadeIn(400);
		l.siblings(".panel").stop().removeClass("expanded").addClass("compressed").animate({width: "18%"}, 700);
		...
		f(".panel").stop().animate({width: "33.3%"}, 700, function() {
		...
		f(".panel-title").stop(true, true).fadeOut(200);
		l.stop().removeClass("compressed").addClass("expanded").animate({height: "22em"}, 700);
		l.siblings(".panel").stop().removeClass("expanded").addClass("compressed").animate({height: "3em"}, 700);
		f(".panel-content", l).stop(true, true).delay(400).fadeIn(400);
		l.siblings(".panel").find(".panel-content").stop(true, true).fadeOut(400, function() {
			f(this).delay(500).removeAttr("style")
		});
		...
		f(".panel").stop().animate({height: "5.5em"}, 700, function() {
			f(".panel-title").fadeIn(250)
		}).removeClass("expanded compressed");
